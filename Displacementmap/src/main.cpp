...
int main()
{
    ...
    window = glfwCreateWindow(1000, 1000, "Displacement Mapping", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
        return -1;
    ...
    const GLchar *vertexSource = R"glsl(
        ...
        void main(){
            ...
            vec3 displacement = texture( DisplacementTextureSampler, vertexUV ).rgb * 2.0 - vec3(1.0,1.0,1.0);
            float height = 0;
            if (displacement.x >= 0 ) {
                height = length(displacement) * 0.02;
            } else {
                height = length(displacement) * -0.02;
            }
            gl_Position =  MVP * vec4(vertexPosition_modelspace + height * vertexNormal_modelspace,1);
    ...
    const GLchar *fragmentSource = R"glsl(
        ...
        void main(){
            ...
            vec3 MaterialSpecularColor = vec3(2.0,2.0,2.0) * MaterialDiffuseColor;

            // Local normal, in tangent space. V tex coordinate is inverted because normal map is in TGA (not in DDS) for better quality
            vec3 TextureNormal_tangentspace = normalize(texture( NormalTextureSampler, vec2(UV.x,UV.y) ).rgb*2.0 - vec3(1.0,1.0,1.0));
