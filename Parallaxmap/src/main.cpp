...
int main()
{
    ...
    window = glfwCreateWindow(1000, 1000, "Parallax Mapping", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
        return -1;
    ...
    const GLchar *fragmentSource = R"glsl(
        ...
        void main(){
            ...
            // height = depth(u,v)
            vec3 displacement = texture( DisplacementTextureSampler, UV ).rgb * 2.0 - vec3(1.0,1.0,1.0);
            float height = 0;
            if (displacement.x >= 0 ) {
                height = length(displacement) * 0.005;
            } else {
                height = length(displacement) * -0.005;
            }
            ...
            float thetau = acos(dot(tangent, EyeDirection_tangentspace) / length(EyeDirection_tangentspace));
            float thetav = acos(dot(bitangent, EyeDirection_tangentspace) / length(EyeDirection_tangentspace));

            float u_prime = UV.x + tan(thetau) * height;
            float v_prime = UV.y + tan(thetav) * height;

            // Material properties
            vec3 MaterialDiffuseColor = texture( DiffuseTextureSampler, vec2(u_prime, v_prime) ).rgb;
            vec3 MaterialAmbientColor = vec3(0.3,0.3,0.3) * MaterialDiffuseColor;
            vec3 MaterialSpecularColor = vec3(0.3,0.3,0.3) * MaterialDiffuseColor;
